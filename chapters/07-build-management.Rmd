...
- 4) deploy a GitHub Action that performs the R CMD check every time they do a Git commit. 
...
2. Add a function called `FIXME`
  - `usethis::use_r("FIXME")`
3. Load all the functions and see if the `FIXME` function works.
  - `devtools::load_all()` 
4. Add and build documentation for the `FIXME` function.
  - `devtools::document()`
