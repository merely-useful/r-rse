# Collaborating with Git and GitHub {#github-collaboration}
...
- Differentiate forking and branching and identify situations in which to use each
...
### Exercise: Consider ways to collaborate {#ex-ways-to-collaborate}

Before we get into this section, let's do an exercise to start thinking about 
the different ways people collaborate together,
ranging from writing files and emailing them around to using Google Drive or 
similar services.
...
feel would help when making an R package with others.
...
you do this by *merging* the branches together. In team settings, working on
...
stored on GitHub. Both the original and fork are on GitHub.
...
contributors review, discuss, request further changes, and approve the
changes for merge into the main branch. Think of pull requests like peer
...
not using or using a fork-based approach. For non-fork-based workflows, the
steps are to:
...
1. Pull request gets either accepted and merged or changes are requested, which
...
The main differences between using a non-fork-based approach compared to a
fork-based approach is determined by whether you have access to modify ("write
...
so in that case you would use a non-fork-based approach.
...
likely to use either fork or non-fork-based workflow since you can easily work with Git without ever making
...
### Exercise: Test your knowledge of pull requests and remotes {#ex-mcq-pr-remotes}
...
TODO: gitcreds might be phased out in favor of credientials (https://docs.ropensci.org/credentials/reference/set_github_pat.html) as explained here: https://usethis.r-lib.org/articles/articles/git-credentials.html#ongoing-pat-maintenance
...
TODO: Should this be in scope of the book? How do we make sure to encourage best practices (including security) while staying in scope?
...
Collaborating through GitHub with forks, branches, and pull requests becomes
easier the more you practice. But even then, 
...
In general, the usethis PR helper functions can be grouped into a few categories
or "phases" of the workflow based on your role as a **contributor**, which are
listed below. This list of helper functions might feel overwhelming, but we will
cover them slowly and, as with Git commands, you'll get plenty of practice using
them especially in your team project.

1. Starting a contribution and submitting a pull request:
    ...
    forking and cloning (downloading) the R package to your computer. For this
    book we won't use it, since we already have a package we are developing.
    - `pr_init()`: Start a new Git branch in the package repository on your
    computer, while in the RStudio R Project.
...
- Receiving comments or edits and making subsequent changes based on the comments:
    ...
    this function.
    - `pr_merge_main()`: Depending on how often others contribute to the
    repository, you might need to update your branch with changes that have
    happened in the main branch of the repository. This function checks if your
    branch has any differences with the main branch on the computer and on the
    remote GitHub repository, and if there are differences it will merge in those
    changes into your current branch.
    - `pr_pull()`: If changes have been made on your PR branch on the remote 
    GitHub repository, you will need to update your local branch by pulling
    from the remote branch. This function does that for you, by checking if
    anything has changed on the remote branch and then will pull and merge those
    changes into your local branch.
    - `pr_push()`: Already described above, you would use it again after making
    changes based on the feedback you get.
- Having the pull request accepted and merged or to be rejected and closed:
    - `pr_finish()`: If the PR has been accepted and merged, use this function
    to switch to the main branch, pull and merge the new changes on the remote
    main branch into the local main branch, and then deletes your local PR branch.
    - `pr_forget()`: If the PR was rejected and you don't need to work on your
    branch anymore, this function will switch back to the main branch and
    delete the PR branch for you.
...
TODO: Fix path of contributing file, might be in .github/
While in the zipfs package R Project in RStudio, type in the Console:

```{r cs-first-pr-init}
pr_init("contributing-using-usethis")
```

This creates a new Git branch called `contributing-using-usethis`. We gave the
new branch this name because we want add some documentation on using the usethis
package to contribute to the package, and it is important that even branch names
be descriptive as well. We can confirm that this branch was created by using
the [gert][pkg-gert] package to list branches by typing in the Console:

[pkg-gert]: https://docs.ropensci.org/gert/index.html

```{r cs-show-created-branches}
gert::git_branch_list()$name
```

This function returns a tibble (data frame) with the `name` column listing
the names of the branches. As we see, it shows the branch we just created.

Next, open up the `CONTRIBUTING.md` file, go to the bottom of the file and add 
these lines of text to it:
...
After adding this text to the `CONTRIBUTING.md` file,
open the RStudio Git Interface and add then commit the changes you made to the
file. Write a message like "`r commit_msg`". After you've done that,
in the Console run the `pr_push()` helper function:

```{r cs-first-pr-push}
pr_push()
```

This opens up the GitHub website with the newly started pull request. To finish
the pull request submission, you need to continue on by clicking the green 
"Create pull request".
TODO: There is probably more to click.
Your PR is now complete and ready to be reviewed.
If this was on a real project, you'd have to wait for feedback on your PR. 
While you wait, you can pause your work on the PR by using:

```{r cs-first-pr-pause}
pr_pause()
```

This function switches from your PR branch to the main branch. Since you are the
only one working on this package, to mimic someone providing feedback or making
some edits, you will do that yourself instead. In GitHub, go to the pull request
you just submitted and click the "Files changed" tab. Scroll to the changed 
CONTRIBUTING file and click the three dots menu on the right side to then select 
the "Edit file". Make a

TODO: Add pics? Or the use icons to mimic the tab? Might be easier to update if 
GitHub changes it.
TODO: Make a video associated with this book and link to it here? (or only link to it for PDF version?)
...
It might occasionally happen where you've created a new branch to make a PR
of a change or have already submitted a PR and realize the change either was
already done in a different file or is not needed anymore. In this case, you
need to get rid of the change made on your computer by using the `pr_forget()`
function. Let's create a new PR branch:
...
Then we run `pr_forget()` to remove the branch and switch over to the main
branch:
...
### Exercise: "Forgetting" a branch that has some changes {ex-pr-forget}

Complete these steps to practice being familiar and comfortable with removing
unneeded branches and changes.

1. Create a branch called `experiment` with `pr_init()`. 
2. Create a new article vignette called `experiment.Rmd` by using `use_article()`. 
3. Add your name to author field of the YAML header of the file and save the file.
4. Using Git, commit the changes you've made.
5. Lastly, remove the changes by running `pr_forget()`.

```{r sol-deleting-unneeded-pr-branch, echo=TRUE}
pr_init("experiment")
