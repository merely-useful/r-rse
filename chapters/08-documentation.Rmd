...
contains the following helpful tips about the reasons to use an R
markdown README instead of just writing it in markdown directly:
...
> <https://github.com/r-lib/actions/tree/master/examples>. You can also
...
text. <!-- what's the difference between build_readme() and knit?-->
...
title and creates a short template R markdown document in the
`vignettes` folder.
...
[simple](http://127.0.0.1:28784/library/devtools/doc/dependencies.html)
or
...
one-paragraph description of the purpose of `zipfs`, and an example of
how to use the `zipfs` functions together to analyze a book. Knit the
file to preview the vignette.
...
Run `style_pkg()` to restyle the files in the `zipfs` package. Use Git
to explore some of the changes that were made. What do you notice?
