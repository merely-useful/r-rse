...
* Documentation workflow: documenting the small stuff ([R Packages reference](https://r-pkgs.org/man.html?q=roxygen#man))
  ...
  * Code: function documentation cycle. Add `roxygen` comments (with cursor in function definition, do `Code > Insert roxygen skeleton` in RStudio). Run `devtools::document()`. Include a deliberate typo here that will get caught by `spelling` later in the chapter?
