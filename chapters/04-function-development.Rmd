...
* Text: What changes? Adds line to Imports in DESCRIPTION, ensures when someone installs our package, they get tibble too if they don't have it.
...
Text: Also need to be explicit when we use tibble functions
that these come from the tibble package, use `tibble::` in front
...
Text: Still have an error, but now it's related to functions we use inside `extract_element()`.

### Exercise: Fix dependency problems in `extract_element()`

Reminder of steps:

* Add the stringr package as a dependency with `usethis::use_package()`
* Anywhere a stringr function is used preface with `stringr::`

You'll know it's worked if this code works without error in a fresh session of R.
...
generates `man/` and allows `?function_name`, Maybe explain all this using an existing package.
...
2. Run `document()` to generate help page and preview with `?count_words`
...
* Text: `@export`, why we might not document every function., e.g. `extract_element()`
...
* Text: As a comment? Mention other kinds of process control, give pointer to learn more.
...
See if you want fancier bulleted list type error messages: https://rlang.r-lib.org/reference/abort.html
...
Suggest input check could be its own function. Then describe steps as a review of chapter: decide on a name, decide on a file to store it, write it, try it, careful with dependencies, document it.
