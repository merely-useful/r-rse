...
-   Learn about and use the R packages `{devtools}` and `{usethis}` to
    automate and streamline many package development tasks.
-   Learn how to create a new R package using
    `usethis::create_package()`.
-   Describe and explain the purpose of the folders and files in a
    skeleton R package.
-   Describe what are global defaults in R and how to use them to make
...
TODO: Include link to our resources section that has links to books like
R packages, mastering software dev etc.
...
Now, you'll mostly encounter people use an Integrated Development
...
-   There must be an `R/` folder. This will usually contain only `.R`
    ...
    cover functions in more detail in Chapter @sec-function-development.
...
Chapter @sec-function-development and @sec-package-documentation.
...
2.  In this new file you would start creating and developing the
    ...
    detailed function workflow in later chapters, such as Chapter
...
3.  Once you feel the function is ready to be tested out, you might test
    ...
    will talk about more in Chapter @sec-function-development. To make
    the function available to R, you would use `Ctrl-Shift-L` while in
...
documented properly, which we will cover in Chapter
