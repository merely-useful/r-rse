...
object or another dataset, is then done by adding more to the scripts in
`data-raw/`, including the `use_data()` function at the end, and running
the script to get the data as a `.rda` file in the `data/` folder.

### Exercise: Include Frankenstein in the books data

One book is nice to include, but let's add another one to make it more
interesting. Let's add the book "Frankenstein".

1.  Add URL (show what it should look like now)
2.  Re-run the script
3.  `load_all()` and check `names(books)`
