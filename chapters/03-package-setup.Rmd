...
custom company or academic environments, packaging data in a package can
make it very easy to share data with colleagues. And considering R and
many R packages are designed around working with data in some way, like
using statistics or making plots, we will also bundle data with our
...
use_data_raw("gutenberg-project")
```

```{r cs-LABEL-NAME, engine.opts=list(file=fs::path(temp_pkg, "data-raw", "gutenberg-project.R"))}
# DOI: https://doi.org/10.6084/m9.figshare.13040516.v1
download.file(
    "https://figshare.com/ndownloader/articles/13040516/versions/1",
    destfile = here::here("data-raw/books.zip")
)
zip::unzip(here::here("data-raw/books.zip"), exdir = here::here("data-raw"))
fs::file_delete(here::here("data-raw/books.zip"))
```
